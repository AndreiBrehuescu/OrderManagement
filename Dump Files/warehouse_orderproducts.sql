...
CREATE TABLE `orderproducts` (
  `idOrderProducts` int(11) NOT NULL AUTO_INCREMENT,
  `idProduct` int(11) DEFAULT NULL,
  `idOrder` int(11) DEFAULT NULL,
  `quantity` int(11) DEFAULT NULL,
  `total` float DEFAULT NULL,
  PRIMARY KEY (`idOrderProducts`),
  KEY `fk_OrderProduct_product` (`idProduct`),
  KEY `fk_OrderProduct_order` (`idOrder`),
  CONSTRAINT `fk_OrderProduct_order` FOREIGN KEY (`idOrder`) REFERENCES `orders` (`idOrder`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `fk_OrderProduct_product` FOREIGN KEY (`idProduct`) REFERENCES `product` (`idProduct`) ON DELETE CASCADE ON UPDATE CASCADE
...
-- Dump completed on 2020-04-15 17:59:51
